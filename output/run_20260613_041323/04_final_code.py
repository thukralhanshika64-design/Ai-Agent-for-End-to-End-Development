...
def register():
    ...
    if len(username) > 100 or len(password) > 100:  # Check if username or password is too long
        return jsonify({'message': 'Username and password must be less than 100 characters'}), 400
...
def create_reminder():
    ...
    reminder_date = request.json['reminder_date']
    try:
